defmodule ExFixerTest do
  ...
  test "warnings", %{filename: filename, compilation_results: compilation_results} do
    warnings = warnings(compilation_results)
  ...
  test "fix_unused_variable" do
    assert fix_unused_variable("sample", "def execute_for_sample(sample) do") ==
             "def execute_for_sample(_sample) do"

    assert fix_unused_variable("symbol", "def execute_for_sample(%{symbol: symbol}) do") ==
             "def execute_for_sample(%{symbol: _symbol}) do"
  end

  test "fix_unused_atom" do
    assert fix_unused_atom("CoinHunter.Repo", "alias", "alias CoinHunter.Repo") == ""
    assert fix_unused_atom("Account", "alias", "  alias CoinHunter.Model.{Account}") == ""
